...
export default function PassForm() {
  ...
  const PRIX = isAdherent === true ? 25 : 49
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div>
          ...
          {!checkingEmail && isAdherent === true && (
            <div className="mt-2 flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
              ...
              <span className="text-xs font-semibold text-green-700">Adhérent AUNL — tarif préférentiel 25€</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        <div className="flex justify-between mb-2 text-sm">
          <span className="text-gray-600 font-medium">Pass 6 séances / 30 jours</span>
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-2xl font-bold tracking-tight">{PRIX}€</span>
        </div>
        <p className="text-xs text-gray-400 mb-5">Paiement unique · Aucun prélèvement automatique</p>
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? 'Redirection…' : `Acheter mon pass — ${PRIX}€ →`}
        </button>
